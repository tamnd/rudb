...
//!
//! What it does not do yet is a `LIMIT` or `OFFSET` holding a subquery, `CREATE TEMPORARY TABLE`, `CREATE TEMPORARY VIEW`, and every statement outside the implemented set. Each gap is an error naming what was written rather than a silently wrong plan, which is the rule the whole front end follows.
