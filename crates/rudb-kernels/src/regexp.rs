...
impl Call {
    ...
    fn stable_replace(
        ...
        let cached = self
            ...
            .as_ref()
            .map_err(Clone::clone)?;
        // A selection keeps stable codes and clones this `Arc`. Another stable dictionary can have
        // the same width while assigning entirely different values to its codes, so its length is
        // not an identity. Only reuse a remap for the exact source dictionary that produced it.
        if !Arc::ptr_eq(&cached.source, dictionary) {
            return Ok(None);
        }
...
mod tests {
    ...
    #[test]
    fn stable_replacement_does_not_reuse_codes_for_an_equal_width_dictionary() {
        let pattern = Value::Varchar("^https?://(?:www\\.)?([^/]+)/.*$".into());
        let replacement = Value::Varchar("\\1".into());
        let call = Call::read("regexp_replace", &[&pattern, &replacement])
            .expect("valid pattern")
            .expect("a prepared call");
        let dictionary = |values: &[&str]| {
            Arc::new(
                Vector::from_values(
                    LogicalType::Varchar,
                    &values.iter().map(|value| Value::Varchar((*value).into())).collect::<Vec<_>>(),
                )
                .expect("dictionary values"),
            )
        };
        let first = Vector::stable_dictionary(
            vec![0, 1],
            dictionary(&["https://one.example/a", "https://two.example/b"]),
        )
        .expect("first stable dictionary");
        vectorized("regexp_replace", Some(&call), &[first], &LogicalType::Varchar, 2)
            .expect("first replacement succeeds")
            .expect("first vectorized replacement");

        let second = Vector::stable_dictionary(
            vec![0, 1],
            dictionary(&["https://three.example/a", "https://four.example/b"]),
        )
        .expect("second stable dictionary");
        let output = vectorized("regexp_replace", Some(&call), &[second], &LogicalType::Varchar, 2)
            .expect("second replacement succeeds")
            .expect("second vectorized replacement");
        assert_eq!(output.value_at(0), Value::Varchar("three.example".into()));
        assert_eq!(output.value_at(1), Value::Varchar("four.example".into()));
    }
}
