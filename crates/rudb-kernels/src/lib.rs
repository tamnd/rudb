...
//!
//! [`compare::compare`], [`scalar::call`] and [`logic::combine`] are done. They dispatch once on the
//! form pair and once on the physical layout, hoist everything that does not change from row to row
//! out of the loop, and keep the old row at a time loop as the oracle their property tests check
//! against rather than as dead code. The kernels that have not been converted yet still run the old
//! loop, and they are still correct.
//!
