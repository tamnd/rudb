...
mod tests {
    ...
    /// A struct left the group in #594, and for a different reason than a list did.
    ///
    /// A gather of a list column is eight bytes a row whatever is in it, and a gather of a struct column
    /// is a gather of every field, so the two are both allowed here and only one of them is cheap. The
    /// guard's job is whether a gather is possible at all, and how much a wide row costs is the cost
    /// model's job, so this passing and the list one passing do not mean the same thing.
    #[test]
    fn a_struct_column_can_be_copied_out_and_costs_what_its_fields_cost() {
        let mut settings = Settings::new();
        settings.pin(SeamId::ChunkCompaction, "fixed-threshold");
        let types = [LogicalType::Struct(vec![
            Field::new("a", LogicalType::Integer),
            Field::new("b", LogicalType::Varchar),
        ])];
        let context = Context::new(SeamId::ChunkCompaction, &settings).with_types(&types);
        let registry = compaction();
        let chosen = registry.choose(&context).expect("a struct can be gathered");
        assert_eq!(chosen.name(), "fixed-threshold");
    }
