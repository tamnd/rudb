...
/// The header of the forward link the child's file holds for that relationship, built against that
/// parent, which says whether every child row found a parent through it and which form it took.
///
/// The same question `rudb_links()` answers in its stored columns, asked here for one relationship
/// at a time. `None` is no link, a header whose `linked` is short of its `children` is a link some
/// of whose children matched nothing, and one where they agree is both certificates of
/// `spec/stats/07-graph-statistics.md` section 7.3. Totality is read off the link's header rather
/// than out of the degree section beside it, because the link counted the children on its way to
/// being written and a file from before that section existed still answers.
///
