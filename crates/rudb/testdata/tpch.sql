-- The twenty two TPC-H queries, exactly as the suite in tamnd/rudb-bench runs them.
--
-- Copied from that suite, which copied them from the specification. Nothing here is
-- rewritten for rudb, for the reason the ClickBench file beside this one gives: a query
-- that needs an edit to run is a compatibility bug and not a dialect difference.
--
-- The newlines are gone and the indentation with them, so a query is one line and a reader
-- can tell where one ends without counting semicolons.
--
-- This is the query list the sections off differential of `spec/graph/09-measurement.md`
-- section 9.2 runs both ways. The data is not here and cannot be, so the tool that reads
-- this file is pointed at a directory somebody generated.
