...
--
-- q28 and q29 say STRLEN and not length because DuckDB's file says STRLEN. The two are
-- different functions upstream, STRLEN counts bytes and length counts characters, and this
-- file carried length for a while, which is why the front end passed all forty three here
-- while the real benchmark could not get past query 28 at all.
