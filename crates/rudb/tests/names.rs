...
const NAMES: &[(&str, &str)] = &[
    ...
    ("count(*) FILTER (WHERE i > 1)", "count_star() FILTER (WHERE (i > 1))"),
    ("count() FILTER (WHERE i > 1)", "count_star() FILTER (WHERE (i > 1))"),
    ("count(i) FILTER (WHERE i > 1)", "count(i) FILTER (WHERE (i > 1))"),
    ("sum(i) FILTER (i > 1)", "sum(i) FILTER (WHERE (i > 1))"),
    ("sum(DISTINCT i) FILTER (WHERE i > 1)", "sum(DISTINCT i) FILTER (WHERE (i > 1))"),
    ("sum(i) FILTER (WHERE b)", "sum(i) FILTER (WHERE b)"),
    ("count(*) FILTER (WHERE i > 1) OVER ()", "count() FILTER (WHERE (i > 1)) OVER ()"),
    (
        "sum(i) FILTER (WHERE i > 1) OVER (ORDER BY i)",
        "sum(i) FILTER (WHERE (i > 1)) OVER (ORDER BY i)",
    ),
    ("sum(i) + 1", "(sum(i) + 1)"),
