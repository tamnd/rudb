...
//! function of the row number, which was checked against DuckDB before it was written down here:
//!
//! ```text
...
//! ```
//!
//! That query returns zero, so the formulas below are DuckDB's reading of the file and not this
//! reader's. A dump would have been this reader's own output blessed as correct, which tests that it
//! keeps doing whatever it does rather than that it is right.
...
fn the_schema_is_the_one_the_footer_describes() {
    ...
    // DuckDB writes every column optional, so nothing here is NOT NULL even though only s has nulls.
    assert!(fields.iter().all(|field| !field.not_null), "{fields:?}");
}

#[test]
fn a_row_group_comes_back_as_whole_vectors_and_a_remainder() {
    let mut reader = reader();
    let lengths: Vec<usize> = chunks(&mut reader).iter().map(Chunk::len).collect();
    // Two row groups of 2048, each cut into two vectors of 1024, which divides exactly.
...
fn a_projection_reads_only_the_columns_it_asked_for() {
    ...
    let everything = all.bytes_read();

    let mut one = reader();
    one.project(&[1]).expect("column 1 exists");
    let chunks = chunks(&mut one);
    let some = one.bytes_read();

    assert!(some < everything, "{some} bytes for one column against {everything} for seven");
    assert!(some > 0, "one column is still some bytes");
    let mut i = 0_i64;
    for chunk in &chunks {
...
fn counting_the_rows_reads_no_column_data_at_all() {
    ...
    assert_eq!(reader.bytes_read(), 0, "a count reads the footer and nothing else");
}
