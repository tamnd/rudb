...
**Three of the overridden names are also written as rules in the grammar, and their written bodies are dead.** `Identifier <- QuotedIdentifier / PlainIdentifier`, `TypeName <- UnreservedKeyword / TypeNameKeyword / Identifier` and `ReservedKeyword`, which the keyword lists define. An override wins, so none of those bodies is ever walked, and building the generator is what turned that from a reading of the C++ into something the table proves: `PlainIdentifier` and `QuotedIdentifier` are reachable from nothing else and do not appear in the compiled table at all. The third one is the one that bites. `ReservedKeyword` is overridden to `ReservedIdentifierMatcher`, which is the matcher that does no keyword check, so `ReservedKeyword` matches any word whatsoever rather than the 75 in the reserved list. That is a superset, and it lands in two places. `DefArgKeyword <- ReservedKeyword` is the acceptance case: an index option argument that is a bare word is taken here, and a parser that compiled the list would refuse it. `ColLabel <- ReservedKeyword / UnreservedKeyword / ColumnNameKeyword / FuncNameKeyword / TypeNameKeyword / Identifier` is the parse tree case: the first alternative now matches every word and the other five never run, so a parser built from the text agrees on what it accepts while disagreeing about which alternative it accepted with. The transformer is keyed on rule name and the differential harness in document 14 compares trees rather than only accept against reject, so the second one is a real difference here even though no query changes its answer over it. Read the grammar text on its own and both of these come out backwards, which is the clearest case for why section 20.4 exists: the facts in it are invisible in the file set that vendoring is supposed to make authoritative.
...
**It never reaches a character level node, and the generator proves that rather than the matcher checking for it.** There are nine character classes and two captures in the vendored tree, and every one of them is inside `%whitespace`, `NumberLiteral`, `StringLiteral`, `PlainIdentifier` or `QuotedIdentifier`: four classes in `NumberLiteral`, two in `PlainIdentifier`, and one each in the other three. The first rule is never referenced because whitespace is applied between tokens rather than called, the next two are overridden, and the last two are reachable only through `Identifier`, which is overridden as well. So the generator walks the graph from `Program` and `TopLevelStatement`, and none of the five is reached: `PlainIdentifier` and `QuotedIdentifier` are not in the compiled table at all, and neither is the grammar's single negative lookahead, which is the `!ReservedKeyword` in `PlainIdentifier`. If a future grammar puts a class or a capture or a lookahead somewhere reachable, generation fails and says which rule, which is a build error on the machine of whoever ran `cargo xtask vendor-grammar` rather than a runtime surprise on somebody's query. The matcher has no character level opcode to reach.

## 20.7 The tokenizer is the part that is still ours

The grammar guarantees less than it looks like it does, and this is the largest of the three gaps. String literals, dollar quoting, numeric literal forms, comments, identifier quoting and the operator rules all live in a hand-written C++ tokenizer that has no declarative artifact. It is 613 lines in `src/parser/peg/tokenizer/base_tokenizer.cpp` plus 45 in `parser_tokenizer.cpp`, a nine state machine over nine token types. We match it by behaviour, and the way we find out whether we have is a differential fuzzer against DuckDB's own tokenizer from week one. Six hundred lines of state machine is the entire distance between us and the compatibility claim on this axis, and it is the cheapest place in the project to buy confidence.

The details that have to be right, each of which is a silent compatibility bug if it is not. Every one of these is a fact about DuckDB rather than a design decision of ours, and every one was read out of that file at the pinned commit rather than out of the documentation. Two of the entries below said the opposite when they were written from the documentation, which is the argument for reading the source in miniature.

**Identifiers are matched without regard to case and stored with the case they were written in.** `SELECT A` and `select a` are the same column, and it comes back named however the DDL spelled it. The tokenizer folds nothing at all; it hands over the bytes as written and the fold happens where a name is compared. Quoted identifiers work exactly the same way, and that is the part that is not PostgreSQL: `"A"` and `"a"` are the same column in DuckDB and different columns in PostgreSQL. Standard SQL folds up, PostgreSQL folds down, DuckDB does neither, and the difference is observable in error messages and in catalog output.

**Keyword classification comes from the vendored `.list` files and not from the generated table.** The longest keyword is fifteen bytes, so the fold happens in a fixed stack buffer and a longer word skips the lookup entirely. A word in the reserved list cannot be a bare identifier; a word in one of the other four can be, in the positions the grammar allows. Putting a word in the wrong class produces precisely the failure this whole document exists to prevent. The table is larger than the lists, because it also carries the fifteen soft words, and a soft word is an identifier token here rather than a keyword one. It keeps its index all the same, which is what lets `ORDER BY x ASCENDING` match on the literal without `SELECT ascending FROM t` becoming a syntax error.
...
**Underscore digit separators are accepted only between two digits.** `1_000` is a thousand, `SELECT 1_` is `1` aliased `_`, and `SELECT 1__0` is `1` aliased `__0`. The exponent reads like the same shape of rule and is not. `SELECT 1e` is a single number token `1e`, not `1` aliased `e`, because the tokenizer takes the `e` on the strength of the digit in front of it and only the parser is in a position to object. `SELECT 1e+` is one token for the same reason. What does get handed back is whatever the number could not use once it has actually stopped, and the unwind at that point keeps only digits and dots, so `SELECT 1e+ 1` comes out as `1`, then `e` as a name, then `+`, then `1`. A decimal point is unconditional in the same way, which is why `SELECT 1.2.3` is one number token and not three things.
...
**Parameters are not a token kind.** `$1` is the operator `$` and then the number `1`, `$name` is the operator `$` and then an identifier, and `?` is a single byte operator like any other. The grammar spells all three out of those pieces, which is why the tokenizer never has to tell a parameter from dollar quoting. The one decision it does make is that a dollar quote opens only when the run of word characters after the `$` is closed by another `$` and does not start with a digit, and an unterminated one is the single unclosed construct that is not an error: it comes back as a string token flagged as having run off the end. There is no `:name` form.
...
**The tokenizer raises exactly four errors and no others**, which is worth stating as a closed set because a fifth is a compatibility bug on its own. An unterminated block comment, an unterminated string literal, an unterminated quoted identifier, and the empty quoted identifier `""`. Everything else it can be handed comes back as tokens, however little sense it makes, and the parser is what refuses it. Two of the four are not raised by the tokenizer proper at all: DuckDB puts them in a caller supplied behaviour object, so the same state machine is also what drives autocomplete, where a half typed query is the normal case rather than an error.

Nothing is decoded at this stage. A string keeps its quotes and its escapes, a number keeps its underscores, an identifier keeps its case. Decoding is the transformer's job, and leaving it there is what keeps a token small and the whole token vector in cache. A token is twelve bytes: a kind, a byte of flags for what was in the gap in front of it, a keyword index, and a start and end offset.
...
## 20.12 The table, measured

Section 20.5 said what the generator would produce. This is what it produces, from the vendored tree at the commit in section 20.2. The numbers are here rather than in a commit message because the next person to change the generator needs a before to compare against, and because two of them are not what section 20.5 predicted.

| | |
|---|---|
| rules in the table | 1,088 |
| nodes | 4,720 |
| child list entries | 3,043 |
| distinct operator and punctuation symbols | 55 |
| memoized rules | 22 |
| generated file | 491,624 bytes over 11,113 lines |

**The rule count went up rather than down, and both directions are worth following.** The grammar has 1,087 rule arrows, being the 1,086 rules of section 20.2 plus `%whitespace`. Five of those never reach the table. `List` and `Parens` are expanded at each of their call sites and so are never rules of their own. `%whitespace` is the runtime's rather than a rule. `PlainIdentifier` and `QuotedIdentifier` are unreachable, for the reason section 20.6 gives. That leaves 1,082, and six more come from outside the `.gram` files: the five keyword class rules, which are defined by `keywords/*.list` and are referenced by the grammar as though they were ordinary rules, and `EndOfInput`, which `TopLevelStatement <- Statement? (';'+ / EndOfInput)` requires and which section 20.4 deliberately keeps out of the vendored override list. The generator synthesizes that one node rather than the list carrying it, which keeps the extraction defined as exactly the contents of upstream's marked block.

**Four of the five class rules compile to a mask test and the fifth does not**, which is the second thing section 20.5 got wrong by a little. `UnreservedKeyword`, `ColumnNameKeyword`, `FuncNameKeyword` and `TypeNameKeyword` each become one node that asks whether the token's class bits intersect a constant, instead of an ordered choice of up to 339 alternatives. That is exactly equivalent because the words in a list are distinct and the matcher compares them without regard to case, so no alternative can shadow another and the order they are tried in is not observable. `ReservedKeyword` is not one of the four, because it is an override, so the mask it would have had is never built. Section 20.4 is where that ends up mattering.

**The node mix**, which is the useful shape when reading a diff of this file. 1,779 references to a rule, 779 sequences, 728 keywords, 544 optionals, 487 symbols, 250 choices, 124 repetitions, 21 identifier matchers, 4 keyword class tests, and one each of number, string, operator and end of input. So a little over a third of the table is a reference to another rule, which is what a grammar factored into 1,086 named pieces looks like from underneath, and it is also why the first token filter is worth what section 20.8 says it is: every one of those references is a node the filter can decline to walk after reading eight bytes.

**A node is twelve bytes and its FIRST set is another eight**, in a parallel array at the same index rather than in the node, so the filter reads only `FIRST` and never pulls a node into cache to reject it. The 64 bits are six kinds and 58 keyword buckets, the bucket being the keyword's index in the table modulo 58. A token maps to exactly one bit. With 514 words over 58 buckets a keyword bit stands for about nine words, so a bit set for `SELECT` is also set for eight other words, and the filter can say yes to a token that then fails. That direction is free, costing one walk that was going to happen anyway. The other direction is not, and the invariant the generator maintains is that a clear bit can never match, which is why anything it cannot work out resolves to every bit set.

**One thing the generator refuses that upstream does not check for.** A repetition whose body can match the empty string is an infinite loop, and upstream's `RepeatMatchProcess` has no empty match guard, so a grammar with that shape hangs DuckDB rather than failing it. The nullable analysis that computes the FIRST sets already knows which bodies those are, so the check costs nothing and generation fails naming the rule. The vendored grammar has none, which is the answer we expected and not one worth assuming would stay true.

## 20.13 What walking it actually cost

Section 20.6 said what the matcher would be. This is what it turned out to be, and the two things about the dialect that only showed up once something was walking the table over real tokens.

**A `.` between two names is a number token.** `a.b` tokenizes as an identifier, a number whose text is `.`, and an identifier, because `.` is a valid first character of a numeric literal and the scan cannot know whether it has `a.b` or `a` followed by `.5` until it has read the next byte. That is upstream's tokenizer and it is not a bug in ours. It explains a rule in `NumberLiteralMatcher` that looks arbitrary until you know this, which is that a token consisting of a lone `.` is rejected as a number, and it forces a matching decision here: a symbol node compares the token's text and does not check its kind. A kind check would look obviously right and would make `DottedIdentifier <- Identifier DotColLabel*` unmatchable, so every qualified name in the language would be a syntax error. Nothing is lost by dropping it, because every symbol in the table is punctuation, no word or literal has punctuation for its whole text, and a quoted identifier or a string carries its quotes in its text and so cannot collide either. The FIRST solver has to know it too: the entry for `.` is the number bit and the operator bit rather than the operator bit alone.

**The keyword category that decides whether a word can be a function name is not a sixth list.** `IdentifierMatcher` asks `IsKeywordOfCategory` with `KEYWORD_TYPE_FUNC` at a function name position, which reads `typefunc_keyword_map`, and the question of what fills that map is not answerable from the five `.list` files. `ParsedGrammarKeywordHelper`'s constructor answers it: a table of five rule names against five keyword sets, where the entry for that map names the rule `TypeFuncKeyword`, which the grammar defines as `TypeFuncKeyword <- TypeNameKeyword / FuncNameKeyword` and which the constructor then walks through its references collecting literals. So the category is the union of two of the five lists, written once in the grammar, and it cannot drift from them. That is the answer we had guessed, and it is worth having checked, because the other plausible answer would have made `SELECT binary(x)` and `SELECT CAST(x AS binary)` disagree about a word for no reason a reader could find.

**The parse is already linear, and the cost is the table's shape rather than backtracking.** A 56 token ClickBench query goes into 4,925 table nodes, which is 88 a token, and no rule is entered more than 32 times. There is no exponential blowup to fix and the 22 memoized rules are not carrying the result. Where the 88 come from is the expression grammar: `Expression` down to `BaseExpression` is about twenty named precedence levels, and every operand in the query walks all of them whether or not an operator at that level follows. That is the grammar's shape, it is what DuckDB does too, and changing it means not vendoring the grammar.

**Twenty nanoseconds a node is the number worth attacking.** 4,925 nodes in 100 microseconds is not a step count problem, it is three separate array reads at the same index for every node entered, in `NULLABLE`, `FIRST` and `NODES`, plus a fourth in `RULES` to find a rule's body and a fifth in `CHILDREN` for a sequence. Those five arrays are about 136 KB together, which does not fit in L1, so an entered node is several L2 accesses and the arithmetic between them is free by comparison. The parallel array in section 20.12 is still right about the node the filter rejects, which costs one read and no node load. It is the node the filter accepts that pays three, and on a query that parses most nodes are accepted. The fix is a generator change rather than a matcher change: one entry holding the op, the flags, the two operands, the nullable bit and the FIRST set, with a rule's body index written into the node so the hot path never reads `RULES` at all. That is measured against these numbers when it lands.
