...
A `FILTER (WHERE ...)` on a window call is the same selection document 07 section 7.8 describes, applied to the rows the frame already covers. The frame is found first and the predicate decides which of the rows inside it the call reads, so a filter never moves the frame and never changes which row is the current one. A ranking window reads no values at all, so a predicate over the values has nothing to keep or drop and is refused with the sentence `DISTINCT` on the same call is refused with. The predicate is bound inside the window rather than inside the call, which is the one place this differs from an aggregate: a window written in it is a nested window, while an aggregate written in it is an ordinary aggregate over the same rows.
