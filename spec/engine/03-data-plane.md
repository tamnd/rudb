...
`VECTOR_SIZE` is 8192. DuckDB uses 2048 and Velox uses 1024. This was 1024 and said that neither number had an argument attached to it in this project. #480 attached one, and document 12 section 15 is the measurement.

The argument this section expected to matter was what has to stay resident while an expression tree is evaluated. A five-node expression over 8 byte data at 1024 rows is five intermediates of 8 KB, which is 40 KB, and it does not fit in a 32 KB L1d and does fit in a 128 KB one. At 8192 it is 320 KB and fits in neither. That was the reason to expect a small number to win and it is not what the measurement found: at one thread on twenty million rows, going from 1024 to 8192 takes a filtered `count(*)` from 14.0 milliseconds to 1.9 and `sum(k + v)` from 66.8 to 52.6. The per-call cost of the pipeline was an order of magnitude larger than the cache effect this paragraph was worried about, and it stayed larger until the working set was well past L1.

The place the cache argument does bite is 32768, which was measured too. It wins another few percent on a full scan and loses on the load, on a needle the chunk zone maps would otherwise rule out, and on anything with a string column, where a vector of 16 byte views is half a megabyte. 8192 is where the per-call overhead has stopped mattering and the working set has not started to, and the curve between 4096 and 32768 is flat enough that the choice inside it is about the string case rather than about the scans.
...
This layer does not implement the buffer manager. It puts the enum in with the owned variant present, so that adding a borrowed variant at layer three is a change inside `rudb-vector` and not a change to every signature in the workspace. That is the whole reason it appears in layer one, and the bet has been collected twice since.

The second variant is a run inside a page held by an `Arc`, which arrived before the buffer manager did because the Parquet reader wanted the same thing for its own reason, and it carries an offset and a length rather than being the whole page. The offset is what makes cutting a flat vector free. Every other form can already be cut for nothing, because a bit packed body moves an offset, a dictionary body shares its values and a string body shares its arena, and the flat body was the exception only because a buffer that is a whole allocation cannot name a piece of itself. `spec/perf/12-the-chunk-and-the-page.md` measured what the exception cost, which was 27 percent of a `sum` chunk over an in memory table in allocation and copying. The owned variant does not grow an offset, because a writer appends to it and the way to say a run will be cut many times is to promote it to a page once.
