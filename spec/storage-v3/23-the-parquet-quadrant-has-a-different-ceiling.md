...
**Document 24 profiled it, and the mechanism named in this section is 2.66% of the query.** The hashing is not what the grouping costs. The probe and the byte comparison behind it are 21.5% between them, the kernel's page work behind a table of nearly twenty million string keys is about 15%, and the hash is a rounding error next to either. Scaling by hashes happened to land on the right size because the thing that actually scales with 81.0 million against 21.3 million is the number of lookups, and the hash is one small term inside a lookup. That document also finds that the redundancy this section wants a file wide code space for is almost all present inside a single 32,768 row morsel, so it is reachable without one.

**It then finds that the dictionaries this section proposes to canonicalise are mostly not in the file.** Every `Referer` column chunk in `hits.parquet` is written `PLAIN_DICTIONARY, PLAIN, RLE, PLAIN`, which is a writer that filled its megabyte dictionary page after about ten thousand of the row group's sixty thousand values and wrote the rest as plain bytes. Seven rows in eight arrive as strings and no code space reaches them. The 21.3 million entries this section counts do not exist; the file holds about 2.2 million. The arithmetic in this section should be read as an upper bound on a mechanism that is not available, and the paragraphs above it, which name the stable flag as the whole of the difference, are describing a flag on the eighth of the rows that have codes at all.
