...
## What the change has to be, since the representation is already there

None of this needs a new vector form. `Body::Dictionary { codes, values, stable }` exists in `crates/rudb-vector/src/vector.rs`, a kernel can already return one, and the aggregate already takes a fast path when it sees one. What a scalar function over a dictionary column has to do is return the codes it was given over a value vector the function was applied to, and the reason nobody has written the four obvious lines that do that is that the four obvious lines are wrong in two ways worth naming here, since both are recoverable and neither is visible from the kernel.

The first is that a function is allowed to be many to one. The regular expression in Q29 maps every path under one host to that host, which is the entire point of it, so applying it entry by entry gives a value vector with duplicates in it. `group.rs` groups by code when the key is a stable dictionary, so a duplicated value becomes two groups holding one key, and the query returns the same hostname twice with its rows split between the rows. The fix is that the function's output has to be canonicalised rather than merely computed: deduplicate the results into a smaller dictionary and build a map from input code to output code, which for Q29 turns 2,719,020 entries into the few thousand distinct hosts and makes everything downstream cheaper than the column it came from.

The second is that a dictionary computed per chunk is not the dictionary the aggregate is expecting. `stable: true` is documented as storage guaranteeing one code space across pages, and `group.rs` enforces exactly that by comparing dictionary handles with `Arc::ptr_eq` and raising `an encoded count exchange received two string code spaces` when they differ. A kernel that built a fresh output dictionary per chunk would either trip that check or, worse, set `stable` while every chunk meant something different by the same code. So the application has to be memoised on the identity of the source dictionary and return the same `Arc` to every chunk, which is not an optimisation on top of the change but the condition for the fast path engaging at all.

Put together the shape is one memo keyed by the source dictionary's handle and the function being applied, holding the canonicalised output dictionary and the code map. Per row the kernel then does one array index. Per table it does one pass of the function over the distinct entries and one pass of hashing to canonicalise them, and for Q29 that is 2,719,020 matches and hashes against 8,682,923 matches and 8,682,923 string hashes in the aggregate above. It is the same argument the dictionary makes everywhere else in this system, applied one operator later than it currently stops.
...
**Document 20 tests the assumption under that sentence, which is that the scan is near a floor rather than merely slow.** A scan costs about nine nanoseconds per row per column and costs the same for a two byte column as for an eight byte one, which looks at first like an order of magnitude going spare and is not: a profile puts the decoder at 22% of the scan, so a decoder made infinitely fast is worth 1.3 times rather than ten. The conclusion below survives and the reason changes, from the scan being close to the hardware to the scan being a layered cost with no layer above a quarter.
