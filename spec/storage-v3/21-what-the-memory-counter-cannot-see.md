...
**Both comparisons above are at 10,000,000 rows and the sign reverses at benchmark scale.** Document 22 sets this against document 15, which measures 4.2 GiB against DuckDB's 8.62 at 100,000,000 rows, and finds that the 774 MiB priced here is a fixed overhead: it is 53% of rudb's footprint at ten million and 18% of it at a hundred million, which is the whole of why the ratio differs so much between the two scales. The retention this document measures is still worth removing and it is worth more at small scale than at large. What is withdrawn is the framing, not the measurement: rudb is not behind DuckDB on memory at the scale the target is stated at.

## What this document does not claim

The DuckDB memory figure of 0.44 GiB is document 17's at 10 million rows, which is the scale everything else here was measured at, and document 22 shows that comparing it to anything at 100 million is what went wrong in this section on a host at load 17 to 28 with a disk at 100%. Building a matched DuckDB database was not possible without filling that disk, so the cross engine comparison in the previous section is indicative and the rudb numbers are the ones with a measurement under them. The wall times throughout are inflated by contention and only the comparisons made back to back should be read as ratios.
