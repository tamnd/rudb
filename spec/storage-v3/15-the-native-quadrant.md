...
Document 17 shows that gate is necessary and not sufficient: a copy loaded with a plain `SELECT *`, without the type conversion the published setup performs, holds exactly the right rows and matches on `SUM(UserID)` and `SUM(LENGTH(URL))` to the digit, and still fails eight of the 43 queries in about a tenth of a second each on both engines alike, which a suite total reports as 35 queries of work under a 43 query label. This section should record which load its own copies used.
...
The 4.2 GiB in that table is the row documents 17, 19 and 21 all proceeded without. Those three reason from a 10,000,000 row measurement where DuckDB holds 0.44 GiB against rudb's 1.46 and conclude that memory is the axis where rudb is behind, which is the opposite of what this table says at benchmark scale. Document 22 reconciles the two and re-measures this row at 6.53 GiB on a contended host, still under DuckDB's figure. Anyone quoting a memory number from this series should quote its row count with it.
