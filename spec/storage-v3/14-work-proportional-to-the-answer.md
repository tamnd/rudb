...
*Satisfied, and too weak to be worth satisfying.* Document 16 runs this obligation's own slope test across four string columns of one table at one size, holding the row count fixed so that only `D(Q, c)` and the answer vary, and finds cost tracking `D(Q, c)` to within the spread of the measurement while the answer varies by 5,498x with no effect at all. That is this obligation met exactly as it is written here. It is also 86% of the CPU of `SELECT COUNT(*) FROM hits WHERE URL LIKE '%google%'`, a query whose answer is one row, because `URL` holds 18.3 million distinct values and this obligation bounds the cost by that number rather than by the answer. One evaluation per distinct value instead of one per row is a 5.5x reduction on this column and rudb has already taken it. The bound this document's title states, work proportional to the answer, is four orders of magnitude below the bound O2 actually asks for.
...
**This ranking is superseded by document 16 and the reason is worth keeping.** It reads each query's mechanism off the query text, and three of the nine it lists as O1 are dominated by a substring match rather than by grouping: Q21 is `SELECT COUNT(*) FROM hits WHERE URL LIKE '%google%'`, with no `GROUP BY` and no `DISTINCT` in it at all, and Q22 and Q23 spend more in their `LIKE` than above it. Document 16 measures the operators instead of reading the texts, and finds the suite split roughly evenly between grouping, where rudb already leads 3.03x, and string matching, where it leads 1.38x. It also finds both mechanisms this document prescribes already built, and prices the ranking question: across those eight queries the scan is 16.9% of the CPU, so reducing every operator above it to zero on all of them buys 5.9x there and 5.62x on the suite, against the 10x the project is aimed at. The items above are not alternatives to choose between at that target. They are a sum that has to come out near zero.
