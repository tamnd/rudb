...
13. **Closed runs under a filter, and their extremes, from the profile.** Rewriting q21's `EXISTS` into a join against the least and greatest `l_suppkey` of each order made q21 seven times dearer, and the profile said why: `GROUP BY l_orderkey` over lineitem went through the hash table whenever a filter sat under it or a call was a `min` or a `max`. A filter leaves a flat key as a dictionary over its page, which is not a form `interior` compares, so no run was found and every row was hashed and partitioned. A `min` or `max` was never answered off its run, and a count on its own was kept off that path too. The key is now opened once per chunk for an aggregate that closes runs, a run's least and greatest are read where it lies for whole numbers, dates and decimals up to eighteen digits, and a lone count closes runs like any other call. Measured on the clustered file, grouping lineitem by order under `l_receiptdate > l_commitdate` went from 3.01 to 0.41 billion instructions for a count and from 7.04 to 0.71 for a `min` and a `max`, and a `min` with no filter from 4.86 to 0.35. The suite is where it was, since none of the 22 queries takes this path yet, and every answer is the same.

14. **A join the reduction already answered, from the profile.** On q09 the scan of `lineitem` reads through the adjacency exactly the rows of green parts, 319,404 of six million, and the join to `part` above it then evaluated, hashed and looked up every one of them to find each its part, which nothing above the join reads, and gathered every driving column into a new chunk. The executor now asks, once the build side has finished, whether the scan under a join took the join's own handoff and placed exact rows from it, whether the table holds each key once, and whether the plan that runs reads no column of the gathered side and nothing above reads the join by position. When all of that holds every driving row matches exactly one gathered row, so the chunk goes up as it came with a constant in each gathered column, and in `EXPLAIN ANALYZE` on one thread that join went from 24.8 ms of cpu to 6.5 ms. It is the elimination of `eliminate.rs` for a parent side that is filtered, decided at run time because only then is the reduction known to be exact. Measured against main on both files: q09 from 0.996 to 0.887 on the clustered file and from 1.010 to 0.901 on the base one, q11 from 0.084 to 0.076, the suite from 7.35 to 7.24 and from 9.72 to 9.61, and every answer the same.
