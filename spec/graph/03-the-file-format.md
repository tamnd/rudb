...
As built, the section is the binding the forward link carries, a 32 byte header (children, parents, edges, the width and a layout byte), the offsets bit vector with its rank directory, and the packed neighbours. It is built beside every packed link and never beside a monotone one, because a monotone link already answers the backward question from its own bit vector. The delta form is not built yet. `lineitem → part` at SF1 has thirty children a parent spread over six million rows, so a gap within a list is about two hundred thousand and saves three or four bits of the twenty three, which is not worth a second decoder until a relationship with long, close lists asks for it.
...
Adjacencies are paid for out of a share of their own, **twenty five percent** of the table's stored column bytes, and never out of the ten. A link or a key map costs about what the key column it answers for costs, so a tenth of the table holds several of them. An adjacency lists every child row once, so it costs a row id per child whatever the parent is: `lineitem → part` at SF1 is six million ids of twenty three bits, 18 MB, against a ten percent share of 15.7 MB. Out of the same share it could never be kept, whatever the ranking said. A quarter holds two on `lineitem`, and since every adjacency of one table lists the same children, the ranking among them comes down to the smaller first, which keeps `supplier` and `part` and turns away `partsupp`. Measured on the SF1 files of document 12, the suite moves from 12.34 to 11.52 billion instructions on the clustered file and from 14.68 to 13.82 on the base one, for 64 MB more file on a 280 MB one.
