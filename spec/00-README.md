...
**Vector size is a compile-time constant of 8192, four times DuckDB's 2048.** It is a multiple of the [FastLanes](https://www.vldb.org/pvldb/vol18/p4629-afroozeh.pdf) unit of 1024, so the compression layer and the execution layer still share a granularity, which is what makes compressed execution cheap rather than awkward. It was 1024 until #480 measured it, on the argument that matching the FastLanes unit exactly was what mattered. What actually mattered was that the constant was also deciding how much of a table one zone map covered, and once row groups took that job away the vector was free to be chosen for what an operator costs per call. The DuckDB compatibility layer converts at the boundary. Document 07 justifies this and document 12 covers both the conversion cost and the measurement.
...
`spec/sql/duckdb/` is the implementable version of documents 10, 12 and 14, written after the first month of DuckDB compatibility work, because the way that work was being picked turned out to be wrong in the same shape as before. It was being done one function at a time, which measured against the pinned binary at 1159 function names and a rate of two names a month is twenty years. Document 02 in that folder is the inventory and the arithmetic that says the denominator is about forty families rather than 1159 problems, document 12 is the nine batches in the order their measured value puts them, and document 13 is the checklist the D series milestone issues come from. It also carries the two numbers this document's fourth axis owns, because the harness runs both engines on every record already and recording time and memory there is nearly free, and because ordering a year of work by corpus records without watching the engine is the mistake `spec/engine/` was written to correct.

`spec/graph/`, `spec/stats/` and `spec/bench/tpc-h/` are the join work, written after the ClickBench work had gone far enough to show where the next order of magnitude is not going to come from. The thesis in `spec/graph/` is that an equi join between two tables is a function between two sets of row identifiers, that the function is fixed until the data changes, and that an engine which stores it does not have to recompute it on every query. `spec/stats/` generalises the same idea to every column, so a query with no join in it also gets something back for the bytes, and it carries the feedback rules and the determinism test that keep a learned fact from turning into an unpredictable plan. `spec/bench/tpc-h/` is the measurement, and it is written to be usable before any of the engine work exists, because today the harness refuses TPC-H outright and a suite that does not run cannot say whether anything got faster. The milestones for all three are the G series in the issue tracker, and they run together rather than one after another.
