...
**A string payload that is a page is never copied, whatever is done to the views over it.** A flat string column is a run of views and an arena, which is the same pair the string view form is, so a cut of one, a gather out of one and a flatten of one are all the views and nothing else when the arena is a page somebody else is holding too. Sixteen bytes a row move and the bytes of the strings stay where the page put them, however many times a page is cut, filtered and handed out. A column whose arena is its own still copies, because sharing an owned arena means cloning every byte of it including the bytes of the values nobody asked for, and a producer that means its payload to be read many times says so by putting it on a page.
...
**The compiled baseline is `x86-64-v3`, which is AVX2, BMI2 and FMA.** It is set per target in `.cargo/config.toml`, so the floor under an x86 host is Haswell on Intel and Zen on AMD. This is the thing that gets the vector width, and it gets it everywhere rather than only inside a kernel somebody hand wrote twice: a chunk of `i64` moves four registers at a time instead of two, and an `i64` multiply is one instruction rather than a sequence of 32 bit partial products, which is what q01's `l_extendedprice * (1 - l_discount) * (1 + l_tax)` had been paying. Measured at 12.8 percent of the instructions TPC-H at SF1 retires, with the per query table in `spec/perf/66-the-registers-we-already-have.md`. A `RUSTFLAGS` in the environment replaces the rustflags from a config file rather than adding to them, so anything in this repository that sets one carries the flag too, and the comment in that file says which places those are.

**Runtime SIMD dispatch is for what a baseline cannot reach, which is AVX-512.** `std::simd` is still nightly-only in 2026, so stable code means `core::arch` intrinsics behind `is_x86_feature_detected!` and `std::arch::is_aarch64_feature_detected!`, with the compiled baseline as the fallback. The target that is worth it is AVX-512 and specifically the `VBMI2` instructions that matter for bit unpacking, because that hardware is not general enough to compile for and bit unpacking is a hot enough loop to pay for an indirect call. AVX2 and SSE4.2 are not dispatch targets any more, because the baseline has them. Dispatch is chosen once at startup per kernel family and stored in a function pointer table, which is what ClickHouse does and it works. `spec/19-open-questions.md` Q7 has why this order and not the other one.
