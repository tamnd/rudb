...
16. [`16-a-key-that-is-not-an-integer.md`](16-a-key-that-is-not-an-integer.md) is note 15's defect one operator over. A group key whose type was not one of five went to a run of tagged values, and the compare that runs once per input row per probe step built a value on each side of it, which over a packed column allocates. A date key was nine times behind duckdb and a string key was one and a bit. It also writes down the two ideas that were measured and dropped on the way there, and the hash bug the new test found.
