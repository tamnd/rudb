...
One correction to the paragraph above, added later and worth reading before trusting a small number anywhere in this directory. The counter has to be `instructions:u`, because plain `instructions` counts the kernel's work too and the kernel's work moves with the page cache, which on a shared box is whatever the other tenants left behind. Measured at ring 3 the claim holds exactly and a query lands within 0.03 percent over ten runs, or within 0.08 percent across all 22 at three rounds each. Measured with the default scope, q08 spanned 24 percent across twelve runs of one binary. [`68-the-counter-that-was-not-load-immune.md`](68-the-counter-that-was-not-load-immune.md) has how that was found and what else it broke, including that best of three is the right statistic here and not the median of three.
