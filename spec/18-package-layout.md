...
**The same rule applies to the wrapper, so the wrapper is ours.** The `mimalloc` crate's `GlobalAlloc` answers every allocation with `mi_malloc_aligned(size, align)` whatever the alignment is, and mimalloc only has a fast path for that when the block is small enough to come off a thread's free list of small blocks, which is a kilobyte. A column of a chunk is two thousand values and never is, so every buffer a query took went down a path marked `noinline` that checked the alignment was a power of two, checked the size against the small limit, worked out that a plain allocation would have been aligned enough after all, made one, and checked the answer. Growing had the same shape with a worse ending. `mi_realloc_aligned` passes an alignment of eight or less straight through and takes the slow path at sixteen, and the slow path allocates and copies rather than growing a block where it is, so a `Vec` of anything as aligned as an `i128`, which for the engine means a decimal or an aggregate's accumulator, copied itself every time it grew. `crates/rudb-cli/src/heap.rs` is the same four calls with the alignment decided at the call, where the `Layout` is a constant and the compiler folds the question away. It is worth about a third of a percent of TPC-H at SF1 measured on instructions retired, and it is the only `unsafe` in the shell, which is why the lint there is `deny` and not `forbid` and why the file carries the argument.
