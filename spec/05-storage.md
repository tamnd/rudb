...
**A column with no repeats gets no dictionary, and the decision is made once on the first stripe.** The argument above is about a column whose values come round again, and every part of it stops being true when they do not. Codes as wide as row numbers save nothing on the pages, the payload is the column with an offset table added, and the membership index of a stripe is a list of very nearly every code in the column. So the first stripe of a column is the sample: when nine rows in ten of it held a value that stripe had not seen before, the column is written without a dictionary and that first stripe is encoded again. Once and not per stripe, because a code has to mean the same thing in every page of a column, and near the top of the range rather than in the middle, because what a dictionary buys is worth keeping everywhere it is real. On TPC-H the orders table written on its own goes from 52.3 MB to 41.4 MB, the load from 6.9 s to 5.8 s, and `select o_comment from orders` from 1.810 G instructions to 1.213 G, which is what the rudb parquet reader takes over the same values. Nine and not five, because on ClickBench the widest string column is `Referer` at 0.131 of its first stripe and every other one is below that, so every one of them keeps its dictionary and keeps the group by on codes that a dictionary is there for. On TPC-H `o_comment` and `c_comment` are at 0.97 and are what this catches. `l_comment` is at 0.883 and so keeps its dictionary, and eight was built and measured rather than argued about: it takes `select l_comment from lineitem` from 4.335 G instructions to 3.473 G and the file from 280.2 MB to 260.4 MB, and it takes a `like` over the same column from 3.29 G to 4.27 G, because a dictionary runs the predicate once a distinct value and there are 3.6 M of those to 6.0 M rows. The 22 query suite came out 6.91 s against 7.07 s in favour of nine, so nine stays until there is a reason to prefer one of those shapes.

**A page with no dictionary is one FSST layer and not the full string cascade.** The cascade is what the payload blocks of a global dictionary go through, and pointing it at every page of a column was measured and rejected. It is a search, encoding the page under every candidate it has and recursing into the integer cascade for the lengths of each one, and on the orders table that took the load from 6.9 s to 48.3 s. It read back no faster than the dictionary it replaced either, 1.807 G instructions against 1.810 G, because unpicking a nest of layers a value at a time costs what the dictionary's payload block decode cost. Raw pages of the same column read in 0.686 G and cost 96.4 MB against the dictionary's 52.3 MB, which says the whole of the difference is what the page has to be put back together from rather than anything about the dictionary itself. One FSST layer keeps most of what the cascade found and gives all of that back: decoding it is one pass over the payload into one buffer, the values are laid end to end in it the way the raw form already lays them out, and what the reader hands a chunk is views over that buffer. A page is offered the per page dictionary first, because that is cheaper still and wins on a page whose values repeat, and the FSST form is taken only when it comes out smaller than the raw one.
