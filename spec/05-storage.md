...
**A column with no repeats gets no dictionary, and the decision is made once on the first stripe.** The argument above is about a column whose values come round again, and every part of it stops being true when they do not. Codes as wide as row numbers save nothing on the pages, the payload is the column with an offset table added, and handing a chunk out costs a read of a decoded payload block per row where a column stored as its own bytes hands out views over a page it already has. On TPC-H lineitem that is `l_comment` read in 456 ms against 202 ms for the same values in a parquet file. So the first stripe of a column is the sample: when nine rows in ten of it held a value that stripe had not seen before, the column is written without a dictionary and that first stripe is encoded again. Once and not per stripe, because a code has to mean the same thing in every page of a column, and near the top of the range rather than in the middle, because what a dictionary buys is worth keeping everywhere it is real. A page written this way goes through the same string cascade the dictionary payload blocks go through, so giving up the dictionary does not turn into storing the column raw.
