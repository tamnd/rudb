...
**CPU time** is per-thread, from `clock_gettime(CLOCK_THREAD_CPUTIME_ID)` on Linux and macOS and `QueryThreadCycleTime` on Windows, accumulated per operator. This is the axis that cannot be bought with threads and it is the one the project's resource claim rests on.
