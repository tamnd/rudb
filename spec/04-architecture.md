...
**Parser.** Hand-written recursive descent producing an AST in an arena. Not a generated parser: DuckDB spent years on a bison grammar forked from Postgres and moved to a PEG parser in v2.0 precisely because the generated one was unmaintainable and gave bad errors. We start where they ended up, minus the generator. The AST is arena-allocated with `u32` indices rather than `Box`, which is the standard Rust idiom for tree-shaped data and avoids a pointer chase per node.

**Error recovery is a parser feature, not an afterthought.** The parser continues past an error and reports multiple diagnostics with spans, because the alternative is that a client tool gets one error at a time and the user edits their query fifteen times. Spans are byte offsets into the original text and every diagnostic carries one.
